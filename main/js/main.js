...
allCards(url).then((data) => AllPitsa(data))
...
meatCards(url2).then((data) => meatCardsRoot(data))
...
cheaseCards(url3).then((data) => cheaseCardsRoot(data))
...
qoziqorinliCards(url4).then((data) => qoziqornliCardsRoot(data))
...
spicyCards(url5).then((data) => spicyCardsRoot(data))
...
function itemId(id) {
  ...
  getFormodal(urlModal).then((data) => modalContent(data))
}
...
const getLocol = JSON.parse(localStorage.getItem('list'))
...
function getIDlocol(id) {
  ...
  setToLocol(urlLocol).then((data) => locolAdd(data))
}
